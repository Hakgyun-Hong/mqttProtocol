...
/// <summary>
/// [In-Process Native] H.265 (HEVC) Publisher
/// 별도의 외부 NuGet 패키지 없이 C# 표준 NativeLibrary API와 [DllImport]만 사용하여
/// C# 메모리 내에서 직접 H.265로 압축 인코딩합니다.
...
public class H265TransportPublisher : ITransportPublisher
{
    ...
    public async Task OpenAsync(TransportOptions options)
    {
        ...
        _listener.Start();
        Console.WriteLine($"[H.265 Publisher Native] Listening on port {options.Port}");
        ...
        _ = Task.Run(async () =>
        {
            while (!token.IsCancellationRequested)
            {
                try
                {
                    ...
                    Console.WriteLine($"[H.265 Native] Client connected: {ep}");
                ...
                catch (Exception ex)
                {
                    if (!token.IsCancellationRequested)
                        Console.WriteLine($"[H.265 Native] Accept error: {ex.Message}");
                }
            }
        }, token);
    ...
    public void StartPublishing(byte[] payload, int targetFps, TransportOptions options)
    {
        ...
        Task.Run(async () =>
        {
                    ...
                    lock (_encoderLock)
                    {
                        unsafe
                        {
                            if (_codecContext != null && _inputFrame != null && _outputPacket != null)
                            {
                                ...
                                {
                                    Console.WriteLine($"[H.265 Native] Send frame failed: {sendResult}");
                                }
                            }
                        }
                    }
            ...
            catch (Exception ex) { Console.WriteLine($"[H.265 Native] Publish loop error: {ex.Message}"); }
        }, token);
    }
    ...
    private void InitializeEncoder(TransportOptions options)
    {
        unsafe
        {
            // 1. 적절한 H.265(HEVC) 인코더 찾기 (GPU 가속 혹은 기본 CPU)
            string codecName = options.UseGpu ? (OperatingSystem.IsMacOS() ? "hevc_videotoolbox" : "hevc_nvenc") : "libx265";
            AVCodec* codec = avcodec_find_encoder_by_name(codecName);
            if (codec == null)
            {
                Console.WriteLine($"[H.265 Native] Warning: Custom codec '{codecName}' not found. Falling back to default CPU H.265.");
                codec = avcodec_find_encoder(AV_CODEC_ID_HEVC);
            }

            if (codec == null)
                throw new Exception("H.265 encoder could not be resolved.");
            ...
            av_dict_set(&dict, "x265-params", "bframes=0:rc-lookahead=0:keyint=1", 0);

            int openResult = avcodec_open2(_codecContext, codec, &dict);
            if (openResult < 0)
                throw new Exception($"Failed to open native H.265 codec context: {openResult}");
            ...
            Console.WriteLine($"[H.265 Native] Encoder successfully initialized in-process (Codec: {codecName})");
        }
    }
