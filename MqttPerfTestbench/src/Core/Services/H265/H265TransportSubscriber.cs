...
public sealed class H265TransportSubscriber : ITransportSubscriber
{
    ...
    public async Task ConnectAsync(TransportOptions options)
    {
        ...
        for (int i = 0; i < 10; i++)
        {
            try
            {
                ...
                Console.WriteLine($"[H.265 Sub] Connected to {options.Server}:{options.Port}");
                break;
            }
            catch (Exception ex)
            {
                Console.WriteLine($"[H.265 Sub] Attempt {i + 1} failed: {ex.Message}");
                if (i == 9) throw;
        ...
        _decodeProcess.ErrorDataReceived += (_, e) => { if (e.Data != null) Console.WriteLine($"[ffmpeg-h265-dec] {e.Data}"); };
        ...
        _ = Task.Run(async () =>
        {
            ...
            catch (Exception ex) { Console.WriteLine($"[H.265 Sub] TCP→FFmpeg: {ex.Message}"); }
        ...
        _ = Task.Run(async () =>
        {
            ...
            catch (Exception ex) { Console.WriteLine($"[H.265 Sub] Decode: {ex.Message}"); }
        }, token);
    }
