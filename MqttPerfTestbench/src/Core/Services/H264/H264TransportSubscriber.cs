...
/// <summary>
/// [In-Process Native] H.264 Subscriber
...
public class H264TransportSubscriber : ITransportSubscriber
{
    ...
    public async Task ConnectAsync(TransportOptions options)
    {
        ...
        for (int i = 0; i < 10; i++)
        {
            try
            {
                ...
                Console.WriteLine($"[H.264 Sub Native] Connected to {options.Server}:{options.Port}");
                break;
            }
            catch (Exception ex)
            {
                Console.WriteLine($"[H.264 Sub Native] Connection attempt {i + 1} failed: {ex.Message}");
                if (i == 9) throw;
        ...
        _ = Task.Run(async () =>
        {
                    ...
                    lock (_decoderLock)
                    {
                        unsafe
                        {
                            if (_codecContext != null && _inputPacket != null && _decodedFrame != null)
                            {
                                ...
                                {
                                    Console.WriteLine($"[H.264 Sub Native] Send packet failed: {sendResult}");
                                }
                            }
                        }
                    }
            ...
            catch (Exception ex) { Console.WriteLine($"[H.264 Sub Native] Loop error: {ex.Message}"); }
        }, token);
    }
    ...
    private void InitializeDecoder()
    {
        unsafe
        {
            ...
            if (codec == null)
                throw new Exception("H.264 decoder not found in FFmpeg native libraries.");
            ...
            if (openResult < 0)
                throw new Exception($"Failed to open native H.264 decoder context: {openResult}");

            // 3. 디코더 출력 프레임 및 수신 패킷 메모리 홀절 생성
            _decodedFrame = av_frame_alloc();
            _inputPacket = av_packet_alloc();

            Console.WriteLine("[H.264 Sub Native] Decoder successfully initialized in-process");
        }
    }
